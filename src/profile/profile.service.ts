...
export class ProfileService {
  constructor(private readonly userService: UsersService) {}
    ...
    async read(userId: string): Promise<Profile> {
      ...
      try {
        const {id, refreshToken, password, ...profile} = await this.userService.find(ProfileService, { id: userId });
        return profile;
      } catch (error: any) {
    ...
    async delete(userId: string) {
      ...
      try {
        const {id, refreshToken, password, ...profile} = await this.userService.delete(userId, ProfileService);
        Logger.log(`Profile deleted for user with id ${userId}`, ProfileService.name);
        return profile;
      } catch (error: any) {
        Logger.error(error.message, error.stack, ProfileService.name);
        throw error;
